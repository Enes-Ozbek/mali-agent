...
def test_an_inline_echo_loses_the_question_but_not_the_answer():
    """This used to assert the whole line survived untouched, on the reasoning that a
    first line carrying answer content should not be dropped. That reasoning is right
    and the behaviour was too blunt: only the echoed prefix needs to go. Live replies
    put the echo inline -- "Toplam KDV ne kadar? Toplam KDV/vergi: 101.692,00 TL" --
    and splitting on newlines never saw it. Every word of the answer still survives.
    """
    reply = "Kaç faturası var? Üç tane.\nToplam 9.000,00 TL."
    cleaned = agent.plain_text(reply, question="Kaç faturası var?")
    assert cleaned == "Üç tane.\nToplam 9.000,00 TL."
